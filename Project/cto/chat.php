...
if(isset($_POST['submit']))
{
    ...
    if($result1)
    {
        while ($row1 = $result1->fetch_assoc()){
            ...
            {
                $manuid = $row1['uid'];
                $mfname = $row1['first_name'];
                // echo $manuid;
            }
            if($emp == $row1['email'])
            {
                $empuid = $row1['uid'];
                $efname = $row1['first_name'];
                // echo $empuid;
            }
...
<?php
if($result){
echo "<h1>Communication between ".$mfname." and ".$efname."</h1>";
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()){
        ...
        if($empuid == $row['uid'] && $man == $row['email']){
...
<?php
        ...
        if($manuid == $row['uid'] && $emp == $row['email']){
...
?>
<a id="bn" class="btn btn-info" href="view_user.php">Back</a>
</body>
</html>