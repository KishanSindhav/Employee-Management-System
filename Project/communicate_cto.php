...
if($me=='CTO')
{
  ...
  if(isset($_POST['submit']))
  {
      ...
      if($responce != ''){
        ...
        $sql="INSERT INTO `comm_cto`(`uid`,`responce`,`time`,`is_cto`,`read_flag`) values('$uid','$responce',now(),1,0)";

      
      $result = $conn->query($sql);

      header("Location: http://localhost/project/communicate_cto.php?me_un=CTO&uid=".$uid);
      }
      $conn->close();
  ...
  if(isset($_POST['submit']))
  {
    ...
    while($row=$result->fetch_assoc()){
      ...
      if($responce != ''){
        ...
        $sql="INSERT INTO `comm_cto`(`uid`,`responce`,`time`,`is_cto`,`read_flag`) values('$uid','$responce',now(),0,0)";

      
      $result = $conn->query($sql);

      header("Location: http://localhost/project/communicate_cto.php?me_un=manager");
      }
      $conn->close();
...
<body style="background-color: 	#FDF5E6 ;">
    <?php 
        ...
        {
            echo "<h1>CTO</h1>";
            echo"<a id=\"bn\" class=\"btn btn-info\" href=\"manager\index.php\">Back</a>";
        }
        else{
            echo"<h1>".$fn." ".$ln."</h1>";
            echo"<a id=\"bn\" class=\"btn btn-info\" href=\"cto\\view_user.php\">Back</a>";
          }
...
if($me=='CTO')
{
  if ($result->num_rows > 0) {
    ...
    $done = $conn->query("UPDATE `comm_cto` SET `read_flag`='1' WHERE `uid`= '".$_GET['uid']."' and `is_cto` = '0' and `read_flag` = '0'");
...
else
{
  if ($result->num_rows > 0) {
    ...
    $done = $conn->query("UPDATE `comm_cto` SET `read_flag`='1' WHERE `uid`= '".$uid."' and `is_cto` = '1' and `read_flag` = '0'");
...
<!-- manager = http://localhost/project/communicate_cto.php?me_un=CTO&un=manager1 -->

<!-- http://localhost/project/communicate_cto.php?me_un=CTO2023&uid=1 -->
